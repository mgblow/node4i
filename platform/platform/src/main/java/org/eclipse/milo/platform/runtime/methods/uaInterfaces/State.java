...
public class State extends AbstractMethodInvocationHandler {
    ...
    public State(UaMethodNode node) {
        super(node);
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
        try {
            logger.debug("Invoking state method of objectId={}", invocationContext.getObjectId());
            String eventDefinitionId =  inputValues[0].getValue().toString();
            boolean result = UaInterface.getInstance(this.uaMethodNode.getNodeContext()).state(eventDefinitionId);
            return new Variant[]{new Variant(result)};
        } catch (Exception e) {
            logger.error("error in state alert method of objectId={}", invocationContext.getObjectId());

...
public class SendMessageToMqtt extends AbstractMethodInvocationHandler {
    ...
    public SendMessageToMqtt(UaMethodNode node) {
        super(node);
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
        try {
            logger.debug("Invoking SendMessageToMqttDevice method of objectId={}", invocationContext.getObjectId());
            ...
            UaInterface.getInstance(this.uaMethodNode.getNodeContext()).sendMessageToMqtt(ioName,topic,message);
            return new Variant[]{new Variant(true)};
        } catch (Exception e) {
            logger.error("error in invoking SendMessageToMqttDevice method of objectId={}", invocationContext.getObjectId());

...
public class GetOutputFromComponent extends AbstractMethodInvocationHandler {
    ...
    public GetOutputFromComponent(UaMethodNode node) {
        super(node);
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
        try {
            logger.debug("Invoking GetOutputFromComponent method of objectId={}", invocationContext.getObjectId());
            String identifier = inputValues[0].getValue().toString();
            String result = UaInterface.getInstance(this.uaMethodNode.getNodeContext()).getOutputFromComponent(identifier);
            return new Variant[]{new Variant(result)};
        } catch (Exception e) {
            logger.error("error in GetOutputFromComponent alert method of objectId={}", invocationContext.getObjectId());

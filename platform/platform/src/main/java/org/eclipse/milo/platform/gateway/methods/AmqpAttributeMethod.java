...
public class AmqpAttributeMethod extends AbstractMethodInvocationHandler {
    ...
    public static final Argument QUEUE_NAME = new Argument("queueName", Identifiers.String, ValueRanks.Any, null, new LocalizedText("topic"));
    ...
    public AmqpAttributeMethod(UaMethodNode uaMethodNode) {
        super(uaMethodNode);
    ...
    public Argument[] getInputArguments() {
        return new Argument[]{NAME, DISPLAY_NAME, LOCATION, IO_Name, QUEUE_NAME, DESCRIPTION};
    }
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) {
        ...
        String QUEUE_NAME = inputValues[4].getValue().toString();

        String deviceIdentifier = GatewayIdentifier.IO_IDENTIFIER.getIdentifier().replace("{app-name}", APP_NAME).replace("{io-name}", ioName);
        String tagId = GatewayIdentifier.AMQP_TAG.getIdentifier().replace("{app-name}", APP_NAME).replace("{io-name}", ioName).replace("{tag-name}", name);
        ...
        UaObjectNode uaAmqpQueueNode = (UaObjectNode) invocationContext.getMethodNode().getNodeManager().get(new NodeId(appNamespaceIndex, tagId));

        //Check whether it is saving or updating
        if (uaAmqpQueueNode == null) {
            uaAmqpQueueNode = new UaObjectNode(invocationContext.getMethodNode().getNodeContext(), new NodeId(appNamespaceIndex, tagId), new QualifiedName(appNamespaceIndex, tagId), new LocalizedText(displayName));
        } else {
            removeFromLastLocation(tagId, uaAmqpQueueNode);
        }
        int index = -1;
        for (Argument input : this.getInputArguments()) {
            ...
            addProperty(tagId, uaAmqpQueueNode, input.getName(), nodeValue);
        }
        uaAmqpQueueNode.setDisplayName(new LocalizedText(displayName));
        invocationContext.getMethodNode().getNodeManager().addNode(uaAmqpQueueNode);

        addProperty(tagId, uaAmqpQueueNode, "value", new Variant(""));


        deviceNode.addReference(new Reference(deviceNode.getNodeId(), Identifiers.HasProperty, uaAmqpQueueNode.getNodeId().expanded(), true));
        UaFolderNode locationFolder = (UaFolderNode) invocationContext.getMethodNode().getNodeManager().get(Utils.newNodeId(location));
        locationFolder.addOrganizes(uaAmqpQueueNode);
        deviceAttributesNode.addComponent(uaAmqpQueueNode);
        deviceNode.addReference(new Reference(deviceNode.getNodeId(), Identifiers.HasProperty, uaAmqpQueueNode.getNodeId().expanded(), true));
    ...
    protected void validateInputArgumentValues(Variant[] inputArgumentValues) throws InvalidArgumentException {
        ...
        MethodInputValidator.NotNull(this, NAME, LOCATION, IO_Name, QUEUE_NAME);

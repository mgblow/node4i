...
public class EventInfo extends AbstractMethodInvocationHandler {
    ...
    public EventInfo(UaMethodNode node) {
        super(node);
    ...
    protected Variant[] invoke(InvocationContext invocationContext, Variant[] inputValues) throws UaException {
        try {
            logger.debug("Invoking eventInfo method of objectId={}", invocationContext.getObjectId());
            String eventDefinitionId =  inputValues[0].getValue().toString();
            String result = UaInterface.getInstance(this.uaMethodNode.getNodeContext()).eventInfo(eventDefinitionId);
            return new Variant[]{new Variant(result)};
        } catch (Exception e) {
            logger.error("error in eventInfo alert method of objectId={}", invocationContext.getObjectId());

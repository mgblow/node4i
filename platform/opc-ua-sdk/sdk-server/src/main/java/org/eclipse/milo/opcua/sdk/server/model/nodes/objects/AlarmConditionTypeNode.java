...
public class AlarmConditionTypeNode extends AcknowledgeableConditionTypeNode implements AlarmConditionType {
    ...
    public TwoStateVariableTypeNode getEnabledStateNode() {
        Optional<UaNode> component = this.getComponentNodes().stream().filter(new Predicate<UaNode>() {
            ...
            public boolean test(UaNode uaNode) {
                return uaNode.getBrowseName().equals(new QualifiedName(2, "Property/EnabledState"));
            }
        }).findFirst();
    ...
    public void setEnabledState(LocalizedText value) {
        getVariableComponent("http://opcfoundation.org/UA/", "Property/EnabledState").ifPresent(n -> n.setValue(new DataValue(new Variant(value))));
    }
    ...
    public TwoStateVariableTypeNode getActiveStateNode() {
        Optional<UaNode> component = this.getComponentNodes().stream().filter(new Predicate<UaNode>() {
            @Override
            public boolean test(UaNode uaNode) {
                return uaNode.getBrowseName().equals(new QualifiedName(2, "Property/ActiveState"));
            }
        }).findFirst();
        if (!component.isEmpty()) {
            return (TwoStateVariableTypeNode) this.getNodeContext().getNodeManager().get(component.get().getNodeId());
    ...
    public void setActiveState(LocalizedText value) {
        getVariableComponent("http://opcfoundation.org/UA/", "Property/ActiveState").ifPresent(n -> n.setValue(new DataValue(new Variant(value))));
    }
    ...
    public AlarmConditionTypeNode clone() {
        try {
            return this.getNodeContext().getServer().getEventFactory().createAlarmConditionTypeEvent(this.getSourceNodeNode(), true);
        } catch (UaException e) {
            LoggerFactory.getLogger(getClass()).error("Error {} happened while trying to clone {}", e.getMessage(), this.getNodeId());
            return null;
        }
    }
}

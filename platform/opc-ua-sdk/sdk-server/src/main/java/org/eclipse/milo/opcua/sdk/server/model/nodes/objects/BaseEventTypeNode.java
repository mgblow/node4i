...
public class BaseEventTypeNode extends BaseObjectTypeNode implements BaseEventType {
    ...
    @Override
    public UaNode getSourceNodeNode() {
        Optional<UaNode> component = this.getComponentNodes().stream().filter(uaNode -> uaNode.getBrowseName().equals(new QualifiedName(getNodeId().getNamespaceIndex(), getSourceName()))).findFirst();

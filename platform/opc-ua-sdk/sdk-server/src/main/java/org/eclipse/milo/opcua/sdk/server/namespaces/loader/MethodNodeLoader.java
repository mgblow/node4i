...
import org.eclipse.milo.opcua.sdk.server.model.methods.AcknowledgeMethod;
...
class MethodNodeLoader {
    ...
    private void loadNode32() {
        ...
        AcknowledgeMethod acknowledgeMethod = new AcknowledgeMethod(node);
        node.setInputArguments(acknowledgeMethod.getInputArguments());
        node.setOutputArguments(acknowledgeMethod.getOutputArguments());
        node.setInvocationHandler(acknowledgeMethod);
...
//        node.addReference(new Reference(Identifiers.AcknowledgeableConditionType_Acknowledge, Identifiers.HasModellingRule, Identifiers.ModellingRule_Mandatory.expanded(), true));
        node.addReference(new Reference(node.getNodeId(), Identifiers.HasComponent, Identifiers.AcknowledgeableConditionType.expanded(), false));
        this.nodeManager.addNode(node);
    }
